...
fun MealDto.toRecipe(): Recipe {
    ...
    for (i in 1..20) {
        ...
        val ingredient = (ingredientField.get(this) as? String)?.trim()
        val measure = (measureField.get(this) as? String)?.trim()

        if (!ingredient.isNullOrEmpty()) {
            ingredients.add(if (!measure.isNullOrEmpty()) "$measure $ingredient" else ingredient)
        }
    }
